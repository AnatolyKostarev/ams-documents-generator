...
- БД: PostgreSQL + Prisma во всех окружениях; для **development** — PostgreSQL в Docker (`docker compose up -d`). SQLite исключён: JSON-поля (`formFields`, `faq`) несовместимы между SQLite и PostgreSQL на уровне миграций Prisma, что гарантированно создаёт расхождение схем dev/prod.
...
| 6                                               | SEO, rate limit, Метрика, логи, политика конфиденциальности                   | 18–22           |
| 7                                               | Release gate, README, деплой                                                  | 8–10            |
| **T**                                           | **Тестирование** (unit Zod/sanitize + integration API/rate-limit + E2E smoke) | **12–20**       |
| **Итого, если делать всё подряд одним потоком** |                                                                               | **144–184**     |
...
> **Узкое место при параллели — фронтенд:** 4a+4b = **32–38 ч** vs этап 3 = **26–30 ч**. При двух разработчиках бэкенд завершится раньше и будет ждать фронтенд. При найме второго исполнителя приоритет — сильный frontend-разработчик на потоке B.

**Календарь (ориентир для планирования)**

- **Один middle+/senior full-time, последовательно:** **144–184 ч** → **~18–23 рабочих дня** (≈ **4–5 недель** при 8 ч/день), без буфера на правки контента и инфраструктуру.
- **Два разработчика с разделением backend/frontend после этапа 2:** критический путь **116–150 ч** → **~15–19 рабочих дней** (≈ **3–4 недели**) плюс буфер на ревью, хостинг, юридическую вычитку 20 документов. Бутылочное горлышко — фронтенд (4a+4b медленнее бэкенда на 6–8 ч); сильный frontend-разработчик на потоке B сокращает итоговый срок.
...
| ----------- | -------------------- | ------------------------------------------------------------- |
| development | Локальная разработка | PostgreSQL в Docker, моковый PDF без Puppeteer                |
...
- **ADR-001 — Хостинг-платформа:** **Рекомендация: Railway** — нативный Chromium без ограничений по размеру бандла, `lru-cache` rate limit достаточен для single-instance. Vercel: `@sparticuz/chromium` физически не укладывается в лимит 50 МБ compressed без ручного Lambda Layer (Vercel Pro + конфигурация) — риск разблокируется поздно. Фиксировать **до старта этапа 3**.
- **ADR-002 — Session store:** выбрать реализацию до старта этапа 3. Опции: `Map` in-memory (MVP single-instance, нет внешних зависимостей) или Upstash Redis (multi-instance, уже нужен для rate limit при Vercel). Зафиксировать в `apps/web/src/lib/session/`. **Поведение при рестарте/деплое:** `Map` (session) и `lru-cache` (rate limit) сбрасываются — ожидаемо для MVP, задокументировать в README.
...
- `schema.prisma` по ТЗ §3.2; единая схема **PostgreSQL** для всех окружений — миграции dev/staging/prod идентичны, никакого переключения `provider`.
...
> **Предусловие:** тексты `templateBody` и `legalBasis` для 20 документов должны быть подготовлены **до или параллельно** с этим этапом. Если их нет — добавить **6–10 ч** к оценке или выделить отдельный трек контент-подготовки рядом с юр. вычиткой в этапе 2.
...
- **Стратегия ревалидации при добавлении документов:** `generateStaticParams` требует ребилда при новом документе. Для MVP (20 документов, редкие добавления) — полный ребилд через `turbo run build` приемлем. Зафиксировать инструкцию в README; ISR с `revalidate` — post-MVP опция при росте каталога.
...
### Этап 6. Нефункциональные требования (18–22ч)
...
- **Политика конфиденциальности + cookie-уведомление** — **блокер для продакшена**. Яндекс.Метрика собирает данные посетителей; страница `/privacy` и видимое уведомление о cookies обязательны до публикации сайта.
...
| Puppeteer на хостинге                                      | Высокая (Vercel)             | Высокое | ADR-001 склоняет к **Railway** (нативный Chromium, нет ограничений бандла); при Vercel `@sparticuz/chromium` не укладывается в 50 МБ compressed без ручного Lambda Layer на Vercel Pro |
...
| Отсутствие политики конфиденциальности при наличии Метрики | Высокая                      | Высокое | Страница `/privacy` + cookie-уведомление — обязательны **до** публикации; входят в этап 6 как блокер продакшена                                                                        |
| Контент `templateBody`/`legalBasis` не готов к этапу 3     | Средняя                      | Среднее | Запустить подготовку текстов параллельно юр. вычитке в этапе 2; при задержке +6–10 ч к оценке этапа 3                                                                                  |
